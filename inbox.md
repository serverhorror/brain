---
type: inbox
---

# Inbox

## Interesting Texts

This first one is a set of rules that have served me well.
They came to live in various discussions with colleagues.
Not much has changed, but they served well for ~15 years.

--

- [ ] [zk is a command-line tool helping you to maintain a plain text Zettelkasten or personal wiki](https://github.com/zk-org/zk)
- [ ] Link Coding tools / frameworks
  - [ ] [</>htmx](https://htmx.org/attributes/hx-trigger/)
  - [x] [[go/note.foam.md]] with [[managing-go-installations.foam.md]]
  - [ ] gRPC
    - [ ] [gRPC](https://grpc.io/)
    - [ ] [connectRPC](https://connectrpc.com/)

## Reading / Articles

- [ ] [[secure-systems.foam.md]]
- [ ] [[article.software-development-continuous-production.rst]]
- [x] [[if-wasm-existed.foam.md]]
- [ ] Cloud: [[gcp/note.foam.md]]
- [[rules.foam.md]]
- [x] [[as-a-software-developer.foam.md]]
- [x] [[secrets-in-system-engineering.foam.md]]
- [x] [[is-a-software-engineer-an-engineer.foam.md]]
- [x] [Programming Sucks (stilldrinking.org)](https://www.stilldrinking.org/programming-sucks)
- [x] [Minimize Code, Maximize Data (database-programmer.blogspot.com](https://database-programmer.blogspot.com/2008/05/minimize-code-maximize-data.html)

This is a collection of interesting texts that I have come across.
They are mostly copy/pasted from the original source.

- [ ] #TODO: [GitHub - gamontal/awesome-katas: A curated list of code katas](https://github.com/gamontal/awesome-katas)
- [ ] [lsp-pyright (github.com)](https://github.com/emacs-lsp/lsp-pyright)
- [ ] [Pyright (microsoft.github.io)](https://microsoft.github.io/pyright)

<!-- markdownlint-disable MD033-->
<details>
<summary>TODOs, FIXMEs, and BUGs</summary>

## TODOs

```foam-query
filter:
  or:
   - tag: "#todo"
   - tag: "#TODO"
   - tag: "#fixme"
   - tag: "#FIXME"
   - tag: "#bug"
   - tag: "#BUG"
format: table
select:
  - title
  - tags
  - backlink-count
  - outlink-count
sort: title ASC
```

</details>
<!-- markdownlint-enable MD033-->

[rules.foam.md]: misc/rules.foam.md "Rules"
[as-a-software-developer.foam.md]: misc/as-a-software-developer.foam.md "As a software developer"
[secrets-in-system-engineering.foam.md]: misc/secrets-in-system-engineering.foam.md "Secrets in Systems Engineering"
[is-a-software-engineer-an-engineer.foam.md]: misc/is-a-software-engineer-an-engineer.foam.md "Is a Software engineer an engineer?"
[go/note.foam.md]: kb/go/note.foam.md "Go"
[managing-go-installations.foam.md]: kb/go/managing-go-installations.foam.md "Managing Go Installations"
[secure-systems.foam.md]: misc/secure-systems.foam.md "Secure Systems"
[if-wasm-existed.foam.md]: misc/if-wasm-existed.foam.md "If WASM+WASI existed ..."
[gcp/note.foam.md]: kb/gcp/note.foam.md "Google Cloud Platform \[GCP\]"
